...
class ClienteController extends Controller
{
    ...
    public function show(Cliente $cliente)
    {
        ...
        $historial = [];
        if (class_exists(\App\Operations\Infrastructure\Persistence\Eloquent\Cita::class)) {
            $historial['citas'] = $cliente->citas()->latest('inicio')->limit(50)->get();
        }
        if (class_exists(\App\Billing\Infrastructure\Persistence\Eloquent\Factura::class)) {
            $historial['facturas'] = $cliente->facturas()->latest()->limit(50)->get();
        }
        if (class_exists(\App\Shared\Infrastructure\Persistence\Eloquent\Nota::class)) {
    ...
    public function historialPos(Request $request)
    {
        ...
        $cliente = null;
        $vehiculo = \App\Operations\Infrastructure\Persistence\Eloquent\AssetVehicle::where('placa_identificador', 'like', "%{$q}%")->first();
        if ($vehiculo) {
        ...
        $ordenes = \App\Operations\Infrastructure\Persistence\Eloquent\ServiceOrder::where('cliente_id', $cliente->id)
        ...
        $facturas = \App\Billing\Infrastructure\Persistence\Eloquent\Factura::where('cliente_id', $cliente->id)
            ->orderByDesc('fecha')
            ->limit(20)
        ...
        return response()->json([
            ...
            'visitas' => \App\Operations\Infrastructure\Persistence\Eloquent\ServiceOrder::where('cliente_id', $cliente->id)->count(),
            // Ni anuladas ni cotizaciones (BORRADOR) sin confirmar son compras reales.
            'total_comprado' => (float) \App\Billing\Infrastructure\Persistence\Eloquent\Factura::where('cliente_id', $cliente->id)->whereNotIn('estado', ['ANULADA', 'BORRADOR'])->sum('total'),
            'mecanicos' => $mecanicos,
            'ordenes' => $ordenes,
            'facturas' => $facturas,
        ]);
