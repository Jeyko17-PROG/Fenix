...
use App\IAM\Infrastructure\Persistence\Eloquent\UserFuncionalidad;
...
class Funcionalidades
{
    ...
    public static function permitidasPorPlan(?string $plan): array
    {
        ...
        $definidas = $plan ? \App\IAM\Infrastructure\Persistence\Eloquent\Plan::where('nombre', $plan)->value('funcionalidades') : null;
    ...
    public static function estadoEfectivo(User $user, string $clave): string
    {
        ...
        if ($empresa) {
            ...
            $override = \App\IAM\Infrastructure\Persistence\Eloquent\EmpresaModulo::where('empresa_id', $empresa->id)
     ...
     */
    public static function estadoEfectivoEmpresa(\App\Business\Infrastructure\Persistence\Eloquent\Empresa $empresa, string $clave): string
    {
        $defaults = $empresa->tipoNegocio?->modulos_default;
        if (is_array($defaults) && ! in_array($clave, $defaults, true)) {
            return 'DESACTIVADA';
