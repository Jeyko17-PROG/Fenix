...
use App\Billing\Infrastructure\Persistence\Eloquent\MetodoPagoCobro;
...
class MetodoPagoCobroController extends Controller
{
    ...
    public function destroy(MetodoPagoCobro $metodoPago)
    {
        if ($metodoPago->qr_url) {
            Storage::disk('public')->delete(\App\Shared\Infrastructure\StorageUrl::rutaLocal($metodoPago->qr_url));
        }
        $metodoPago->delete();
    ...
    private function guardarQr(Request $request, MetodoPagoCobro $metodo): ?string
    {
        ...
        if ($metodo->qr_url) {
            Storage::disk('public')->delete(\App\Shared\Infrastructure\StorageUrl::rutaLocal($metodo->qr_url));
        }
