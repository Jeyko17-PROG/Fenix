...
use App\Billing\Infrastructure\Persistence\Eloquent\CajaSesion;
use App\Billing\Infrastructure\Persistence\Eloquent\Gasto;
...
class GastoController extends Controller
{
    ...
    public function utilidadDia(Request $request): JsonResponse
    {
        ...
        $ventas = (float) \App\Billing\Infrastructure\Persistence\Eloquent\Factura::whereDate('created_at', $fecha)->whereNotIn('estado', ['ANULADA', 'BORRADOR'])->sum('total');
        $gastos = (float) Gasto::whereDate('fecha', $fecha)->sum('monto');

        // Desglose por medio de pago del día (reporte de cierre).
        $porMetodo = \App\Billing\Infrastructure\Persistence\Eloquent\Factura::whereDate('created_at', $fecha)
